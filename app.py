...
def normalize_for_ocr(img: Image.Image, max_width: int = 900) -> tuple[Image.Image, float]:
    """Create a lightweight OCR preview while preserving the original page."""
    rgb = img.convert("RGB")
    scale = 1.0
...
def _word_records(data: dict) -> list[dict]:
    rows = []
    n = len(data.get("text", []))
    for i in range(n):
        token = _normalise_text(data["text"][i] or "")
        if not token:
            continue
        try:
...
def _term_tokens(terms: Iterable[str]) -> set[str]:
    useful = set()
    for phrase in terms:
        for token in _normalise_text(phrase).split():
            if len(token) >= 4 or token in {"NIT", "EOI", "RFP", "RFQ", "IFB", "CFMS"}:
                useful.add(token)
    return useful


def _is_anchor(word: str, vocabulary: set[str]) -> bool:
    if word in vocabulary:
        return True
    return any(
        len(v) >= 6 and (word.startswith(v) or v.startswith(word))
        for v in vocabulary
        if abs(len(word) - len(v)) <= 2
    )


def smart_notice_crop(
    original: Image.Image,
    data: dict,
    scale: float,
) -> Image.Image:
    """Return the strongest compact tender notice region, not the full page."""
    rows = _word_records(data)
    if not rows:
        return original

    tender_vocab = _term_tokens(TENDER_TERMS)
    service_vocab = _term_tokens(SERVICE_TERMS)
    tender_rows = [r for r in rows if _is_anchor(r["text"], tender_vocab)]
    service_rows = [r for r in rows if _is_anchor(r["text"], service_vocab)]

    if not tender_rows or not service_rows:
        return original

    page_w = max(int(original.width * scale), 1)
    page_h = max(int(original.height * scale), 1)
    candidates = []

    for t in tender_rows:
        tcx = t["left"] + t["width"] / 2
        tcy = t["top"] + t["height"] / 2
        nearby_services = []

        for s in service_rows:
            scx = s["left"] + s["width"] / 2
            scy = s["top"] + s["height"] / 2
            dx = abs(scx - tcx) / page_w
            dy = abs(scy - tcy) / page_h
            if dx <= 0.34 and dy <= 0.24:
                nearby_services.append((s, dx + dy))

        if not nearby_services:
            continue

        nearby_services.sort(key=lambda item: item[1])
        chosen = [t] + [item[0] for item in nearby_services[:8]]
        min_x = min(r["left"] for r in chosen)
        min_y = min(r["top"] for r in chosen)
        max_x = max(r["left"] + r["width"] for r in chosen)
        max_y = max(r["top"] + r["height"] for r in chosen)

        cluster_cx = (min_x + max_x) / 2
        cluster_cy = (min_y + max_y) / 2
        crop_w = max(max_x - min_x + page_w * 0.18, page_w * 0.28)
        crop_h = max(max_y - min_y + page_h * 0.16, page_h * 0.20)
        crop_w = min(crop_w, page_w * 0.62)
        crop_h = min(crop_h, page_h * 0.48)

        x1 = max(0, cluster_cx - crop_w / 2)
        y1 = max(0, cluster_cy - crop_h / 2)
        x2 = min(page_w, cluster_cx + crop_w / 2)
        y2 = min(page_h, cluster_cy + crop_h / 2)

        local_words = [
            r for r in rows
            if x1 <= r["left"] + r["width"] / 2 <= x2
            and y1 <= r["top"] + r["height"] / 2 <= y2
        ]
        local_tender = sum(_is_anchor(r["text"], tender_vocab) for r in local_words)
        local_service = sum(_is_anchor(r["text"], service_vocab) for r in local_words)
        density = len(local_words) / max((x2 - x1) * (y2 - y1), 1) * 100000
        compactness = 1 / (1 + (x2 - x1) / page_w + (y2 - y1) / page_h)
        score = local_tender * 5 + local_service * 4 + min(density, 8) + compactness
    ...
    inv = 1 / scale
    left = max(0, int(x1 * inv))
    top = max(0, int(y1 * inv))
    right = min(original.width, int(x2 * inv))
    bottom = min(original.height, int(y2 * inv))

    if (right - left) > original.width * 0.75:
        centre = (left + right) // 2
        half = int(original.width * 0.31)
        left, right = max(0, centre - half), min(original.width, centre + half)
    if (bottom - top) > original.height * 0.58:
        centre = (top + bottom) // 2
        half = int(original.height * 0.24)
        top, bottom = max(0, centre - half), min(original.height, centre + half)
...
def scan_image(
    ...
    if tender and service:
        ...
        crop = smart_notice_crop(image, data, scale)
...
def show_results(results: list[ScanResult], scan_date: date) -> None:
    ...
    if not matches:
        ...
        st.subheader(f"{len(matches)} tender cutout(s)")
...
def online_tab() -> None:
    ...
    with right:
        max_pages = st.slider("Maximum pages per paper", 6, 24, 12, help="The app now performs one lightweight OCR pass per page.")
        workers = st.slider("Parallel workers", 1, 4, 3, help="Use 2 if the free server restarts; 3 is normally faster.")
        st.info(
            "Fast OCR mode is enabled. Begin with one newspaper and 12 pages; use three workers unless the server restarts. "
